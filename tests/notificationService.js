...
var svc = require("../app/services/notificationService");


describe("Testing the notification service" , function(){
    
    const rand = Math.random(); 
    
    it('Should save a new template' , done => {
        svc.saveTemplate({
            name    : 'test template' + rand, 
            subject : 'test template' + rand, 
            key     : 'test-template', 
            body    : `<mjml>
  <mj-body>
      ...
      <mj-section background-color="#009fe3" hidden="false" vertical-align="middle" name="Section-A" full-width="full-width" padding-bottom="20" padding-top="20">
        <mj-column vertical-align="middle" width="100%">
          <mj-text align="left" color="#000000" font-size="13px" word-wrap="break-word" font-family="Ubuntu, Helvetica, Arial, sans-serif" container-background-color="transparent" vertical-align="middle" padding-left="25" padding-right="25" padding-bottom="0" padding-top="0">
            <p style="font-weight: 400; line-height: 30px; margin: 13px 0px;"><span style="color: rgb(255, 237, 0);"><span style="font-size: 25px;">Dear {{name}}</span></span>
            </p>
            <p style="font-weight: 400; line-height: 30px; margin: 13px 0px;"><span style="color: rgb(255, 255, 255);"><span style="font-size: 25px;">Welcome to {{display_name}}.</span></span>
            </p>
          </mj-text>
...
</mjml>`}).then(data => done()).catch(err => done(err))
    })

    it('Should send a template' , done => {
        svc.sendTemplate({
            key: 'test-template',
            to: 'segevs09@gmail.com',
            data: {
                name: 'fname lname',
                display_name: 'Rotem Dayan' 
            }
